...
Queue should have max size (e.g., 3 items). New items push out lowest priority if full.
  ...
  <verify>TypeScript compiles: `npx tsc --noEmit src/personality/commentary-system.ts`</verify>
...
3. Add `getTimePressurePrompts(): string[]` - for when agent is restless:
  ...
  <verify>TypeScript compiles: `npx tsc --noEmit src/personality/prompts.ts`</verify>
...
```
  ...
  <verify>TypeScript compiles: `npx tsc --noEmit src/personality/index.ts`</verify>
